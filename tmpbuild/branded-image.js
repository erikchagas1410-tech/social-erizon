...
exports.generateErizonAsset = generateErizonAsset;
const node_fs_1 = __importDefault(require("node:fs"));
const node_path_1 = __importDefault(require("node:path"));
const sharp_1 = __importDefault(require("sharp"));
let logoCache = null;
async function generateErizonAsset(content) {
    ...
    const hookText = buildTspans(hookLines, textX, heroTop, titleSize, textAnchor, true);
    const supportText = buildTspans(supportLines, textX, supportTop, supportSize, textAnchor, false);
    const svg = `
...
function buildStatsSvg(content, canvas, accent, accentAlt, centered, baseX, topY) {
    const stats = [
    ...
    return stats
        ...
        const separator = index < stats.length - 1
...
function buildTspans(lines, x, startY, fontSize, textAnchor, accentLastLine) {
    return lines
        .map((line, index) => {
        const dy = index === 0 ? 0 : Math.round(fontSize * 1.04);
        const fill = accentLastLine && index === lines.length - 1
...
function selectCanvasSpec(content) {
...
function shouldCenter(canvas, content) {
    if (canvas.kind === "landscape" || canvas.kind === "story") {
        return false;
    }
    return !["comparacao", "analise", "checklist", "mini_aula", "insight_estrategico"].includes(content.formato);
}
...
function buildEyebrow(content, canvas) {
...
function getBottomInset(canvas) {
    switch (canvas.kind) {
        case "story":
            return 110;
        case "portrait":
...
function getTitleSize(canvas, lineCount) {
...
function getSupportSize(canvas) {
    switch (canvas.kind) {
        case "story":
            return 28;
        case "portrait":
...
function lineLimit(canvas, type) {
    if (type === "hook") {
        switch (canvas.kind) {
            case "story":
                return 12;
...
function loadLogoDataUrl() {
    ...
    const logoPath = node_path_1.default.resolve(process.cwd(), "public/logo-erizon.png");
    if (!node_fs_1.default.existsSync(logoPath)) {
        return null;
    }
    logoCache = `data:image/png;base64,${node_fs_1.default.readFileSync(logoPath).toString("base64")}`;
    return logoCache;
}
